...
# Func. que verifica que al ingresar uno o más caracteres alfanuméricos a
# check_char() esta devuelve el código de error "2" definido para el caso

def test_check_char_c():
    retorno = Métodos.check_char("1V")  # Se ingresa expresión alfanumérica.
    assert retorno == "2"  # El valor esperado es el código de error "2".

# Func. que verifica que al ingresar uno o más caracteres que no sean ni
# alfanuméricos ni letras acheck_char() esta devuelve el código de error "404"


def test_check_char_d():
    retorno = Métodos.check_char("_")  # Se evalúa con un guión bajo.
